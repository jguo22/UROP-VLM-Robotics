...
namespace Unity.Robotics.UrdfImporter.Control
{
    ...
    public class ManualController : MonoBehaviour
    {
            ...
            if (Input.inputString.Length > 0)
            {
                ...
                foreach (char c in Input.inputString)
                {
                    if (c == '\b') // Backspace
                    {
                        ...
                        {
                            targetAngleInputString = targetAngleInputString.Substring(
                                0,
                                targetAngleInputString.Length
                            );
                        }
                    }
                    else if (c == '\n' || c == '\r') // Enter
        ...
        public void UpdateDirectionWithInput(
            ArticulationBody[] robotJoints,
            float targetAngle,
            int jointIndex
        )
        {
            ...
            if (targetAngle < SmallestJointAngle)
                targetAngle = SmallestJointAngle;
            else if (targetAngle > LargestJointAngle)
                targetAngle = LargestJointAngle;
        ...
        private void Highlight(int selectedIndex)
        {
            ...
            DisplaySelectedJoint(selectedIndex);
            Renderer[] rendererList = robotJoints[selectedIndex]
        ...
        {
            Renderer[] materialLists = robotJoints[index]
                .transform.GetChild(0)
                .GetComponentsInChildren<Renderer>();
        ...
        {
            Renderer[] previousRendererList = robotJoints[index]
                .transform.GetChild(0)
                .GetComponentsInChildren<Renderer>();
            for (int counter = 0; counter < previousRendererList.Length; counter++)
            {
                MaterialExtensions.SetMaterialColor(
                    previousRendererList[counter].material,
                    prevColor[counter]
                );
            }
        }
        ...
        public void OnGUI()
        {
            ...
            GUI.Label(
                ...
                "Press left/right arrow keys to select a robot joint.",
                centeredStyle
            );
            GUI.Label(
                new Rect(Screen.width / 2 - 200, 30, 400, 20),
                "Press up/down keys to move " + selectedJoint + ".",
                centeredStyle
            );

            // Display instructions for angle input
            GUI.Label(
                new Rect(Screen.width / 2 - 200, 50, 400, 20),
                "Press 'T' to set target angle for selected joint.",
                centeredStyle
            );
            ...
            {
                GUI.Label(
                    new Rect(Screen.width / 2 - 200, 70, 400, 20),
                ...
                targetAngleInputString = GUI.TextField(
                    new Rect(Screen.width / 2 - 100, 90, 200, 20),
                    targetAngleInputString
                );
                ...
                // Instructions
                GUI.Label(
                    new Rect(Screen.width / 2 - 200, 110, 400, 20),
